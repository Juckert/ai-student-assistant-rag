...
def list_users():
    with get_db_session() as session:
        ...
        return [
            {"id": u.id, "username": u.username, "role": u.role, "display_name": u.display_name, "created_at": u.created_at.isoformat() if u.created_at else ""}
            for u in users
        ]
...
def list_user_chats(user_id):
    with get_db_session() as session:
        chats = session.query(Chat).filter(Chat.user_id == user_id).order_by(Chat.updated_at.desc(), Chat.id.desc()).all()
        return [
            {"id": c.id, "title": c.title, "created_at": c.created_at.isoformat() if c.created_at else "", "updated_at": c.updated_at.isoformat() if c.updated_at else ""}
            for c in chats
        ]
...
def append_message(chat_id, role, content, sources=None):
    with get_db_session() as session:
...
def list_unanswered_questions():
    with get_db_session() as session:
        questions = session.query(UnansweredQuestion).filter(UnansweredQuestion.status == "new").order_by(UnansweredQuestion.created_at.desc(), UnansweredQuestion.id.desc()).all()
        result = []
        for q in questions:
...
def answer_unanswered_question(question_id, admin_user_id, answer):
    with get_db_session() as session:
        q = session.query(UnansweredQuestion).filter(UnansweredQuestion.id == question_id, UnansweredQuestion.status == "new").first()
        if not q:
            return False
        q.status = "answered"
...
def list_answered_questions(user_id):
    with get_db_session() as session:
        questions = session.query(UnansweredQuestion).filter(
            ...
            UnansweredQuestion.student_seen_at == None
...
def get_chat_messages(chat_id, user_id):
    with get_db_session() as session:
        ...
        return [
            {"id": m.id, "role": m.role, "content": m.content, "sources": m.sources or [], "created_at": m.created_at.isoformat() if m.created_at else ""}
            for m in messages
        ]
...
def save_question_answer(user_id, question, answer, sources, chat_id=None):
    with get_db_session() as session:
        if not chat_id:
            chat = Chat(user_id=user_id, title=build_chat_title(question))
            session.add(chat)
            session.flush()
            chat_id = chat.id
        append_message(chat_id, "user", question)
        append_message(chat_id, "assistant", answer, sources=sources)
        return chat_id
