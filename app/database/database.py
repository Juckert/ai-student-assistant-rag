...
class Database:
    ...
    def put_chunks(self, chunks):
        with get_db_session() as session:
            for qa_dict in chunks.get("QA", []):
                session.add(QAChunk(
                    question=qa_dict.get("question_text", ""),
                    answer=qa_dict.get("answer_text", ""),
                    topic=qa_dict.get("question_topic", ""),
                    year=qa_dict.get("question_year", ""),
                    course=qa_dict.get("question_course", "")
                ))
            for doc_dict in chunks.get("Documents", []):
